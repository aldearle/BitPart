...
/**
 * Multi threaded V4. Uses 2D array of longs arranged row wise (for cache coherence).
 *  * Each thread collects results into an ArrayList.
 *  * Phase2 processes row (of longs) at a time.
 *  * Phase 2 and 3 folded together.
 */

public class MainRunnerFileMTV4 extends MainRunnerFile {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        System.out.println("codebase\t" + "MainRunnerFileMTV4" );
...
//        System.out.println("filter_time\t" + filter_time  / (float) queries.size() + " " + filter_time * 100 / total_time + "%" );
    ...
    protected static <T> List<T> doExclusionsMT(final long[][] datarep, final int dataSize,
        ...
        for (int block = 0; block < number_blocks; block++) {  // spark off number_blocks parallel computations
            ...
            futures.add(executor.submit(() -> {
                ...
                return filter(dat,t,cm,q,block_start_in_longs,block_results );
                }));
        };
    ...
    public static <T> List<T> filter( List<T> dat, double t, CountedMetric<T> cm, T q, int block_start_in_longs, long[] block_results ) {
